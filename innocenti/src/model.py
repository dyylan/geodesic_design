...
class QubitNetworkModel(QubitNetwork):
    ...
    def _get_bigreal_matrices(self, multiply_by_j=True):
        ...
        """
        if multiply_by_j:
            return [complex2bigreal(-1j * matrix).astype(np.float64)
                    for matrix in self.matrices]
        else:
            return [complex2bigreal(matrix).astype(np.float64)
                    for matrix in self.matrices]
    ...
    def _build_theano_graph(self):
        ...
        parameters = theano.shared(
            value=np.zeros(len(self.free_parameters), dtype=np.float64),
            name='J',
